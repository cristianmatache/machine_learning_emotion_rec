...
def compare_pred_expect(predictions, expectations):
    ...
    print("Predictions:", predictions.to_string(index=False))
    print("Expectations:", expectations.to_string(index=False))
...
def compute_confusion_matrix(segments, df_labels, df_data, N):
    ...
    for test_seg in segments:
        ...
        for e in ["anger", "disgust", "fear", "happiness", "sadness", "surprise"]:
            ...
            root = decision_tree(train_df_data, set(AU_INDICES), filter_for_emotion(train_df_targets, emotion[e]))
            print("Decision tree built.")
            T.append(root)
